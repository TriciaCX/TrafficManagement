...
public class RoadInfo {
	...
	public static ArrayList<Integer> getLeftLanesLength(Road myRoad,String crossID) {
		if(myRoad==null||myRoad.getRoadID().equals("-1")) System.out.println("road");
		if(crossID==null) System.out.println("crossID");
		ArrayList<Integer> LeftLanesLengthList = new ArrayList<Integer>();
		int LeftLength;
		int index=0;
		//单向道路且车行驶方向与道路方向一致
		if(crossID.equals((myRoad.getFromCrossID()))) {
			for(Lane lane : myRoad.getForwardLane()) { //从road上得到lane
				//如果初始路上没有车
				Car lastCar;
