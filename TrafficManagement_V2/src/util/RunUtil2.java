...
public class RunUtil2
{
			...
			HashMap<String, Road> MapRoad, int t)
	{
		...
		for (i = size; i > 0; i--)
		{
			...
			// 如果从上面这个循环里面出来的话，就说明要么被取完了，要么就是不用取了
		}
		// 每个lane都取完了
	...
	protected static boolean InsertFreshCarToRoad(Car car, Road road, int t)
	{
		 ...
		 * car.setCurToCross(road.getToCross()); 不用更新的信息 car.setFromCross(fromCross);
		 * car.setToCross(toCross); * car.setMaxVelocity(maxVelocity);
		 */
	...
	public static void FourCarStateUnionProcess(LinkedList<Car> carsFour, int t)
	{
		...
		for (Car c : carsFour)
		{
			...
			case (3):
			{
				// 判断一下了,他要么变成1了要么变成2了,后面的所有的车的数字状态也要跟着更新
	...
	/**
	 * 查询当前道路是否是真正有可走的空间，因为需要先走车道号小的车道。 小车道没空间，但车没有更新过直接判为没路；
	 * 没更新有空间则有路；更新过没空间继续查找大的lane； 更新过有空间为有路；
	 * 
	 * @param road
	 *            当前搜索的道路 ;
	 * @param CrossID
	...
	/**
	 * 对当前车辆规划下一道路,2019.3.28
	 * 保证第一条路可走（考虑rest），从car.getCurToCross开始找路径,可以进一步优化：此时第一条路不能走不一定真的在该时刻不能走，需要安排完一些车后可能会有空间
	 * 
	 * @param car
	 *            当前车辆（两个等待状态）
	 * @param maxRoadLength
	 ...
	 * @param preRoad
	 *            车当前所在道路
	 * @return 从unknown中标记第一跳可行节点 ，不标记没有的路，方向不对的路，过来的路
	 *         对于没空间的路，都标记cost，不管是更新过没空间还是没更新过没空间
	 */
	...
	protected static void markNextCross(Road road, Cross s)
	{
		...
		if (!t.isKnown)
		{
			...
			// cost3：拥挤系数，其中CrossInfo.getCrossCarNum(s.getCrossID(),
			// t.getCrossID())表示有多少辆车会通过这个路口
			float sigmoidCrossCarNum = swish(CrossInfo.getCrossCarNum(s.getCrossID(), t.getCrossID()));
			float cost = Main.w[0] * NormalizedRoadLength + Main.w[1] * (1 - NormalizedRoadLeftLength)
	...
	/**
	 * 将State3:firstCar所处的road上的车的状态更新
	 * 需要更新状态信息，不更新位置信息，如果firstCar能过路口，state=1,后面的车state=3;firstCar不能过路口，state=2,后面的车state=4
	 * 
	 * @param firstCarID
	 */
	...
	public static void UpdateCarsAtState1(String firstCarID, int t)
	{
		...
		else if (!firstCar.getCurToCrossID().equals(firstCar.getToCrossID()))
		{
			...
			} else
			{
				...
				{
					// LinkedList<String> carInNextRoadID = getCarInRoad(nextRoad,
					// firstCar.getCurToCrossID());//4.12-我们不能只看没更新状态的车！！！！
					...
					{// case2.1:该road上也没有车 原来写的是carInNextRoadID.isEmpty()，注意我们的firstcar现在还不在这条路上！！！！！
						// 1、设置位置 curPos
						int hasJumpDis = firstCar.getCurPos();
						firstCar.setCurPos(nextRoad.getRoadLength() - (firstCarSpeed - hasJumpDis));
						// 2、设置状态
					...
					{ // case2.2:该road上有车
						...
						{ // 只有true车
							preRoadCarID = getFirstTrueCarInRoad(nextRoad, firstCar.getCurToCrossID()); // 方法参数 road
																										// 当前道路, @param
																										// crossID
																										// 车从哪个路口到这个路;
						}
						...
						// int car1Speed = Math.min(car1.getMaxVelocity(),
						// nextRoad.getMaxRoadVelocity());
						// int preRoadCarSpeed = Math.min(preRoadCar.getMaxVelocity(),
						// nextRoad.getMaxRoadVelocity());
						// //能过路口，且位置在preRoadCar前面，且preRoadCar不能过路口,将car1变为preCar
						// if(car1.getCurPos()<car1Speed &&
						// car1.getCurPos()<preRoadCar.getCurPos()&&preRoadCar.getCurPos()>=preRoadCarSpeed)
						// {
						// preRoadCar = car1;
						// }
						// }
						...
						{ // 如果preRoadCar是true车，需要进一步判断能否过路口，curPos，laneID
							// 得到preCar之后，首先看firstCar能不能过路口
							if (firstCar.getCurPos() < firstCarSpeed)
							{ // firstCar能过路口
								if (preRoadCar.getCurPos() < preRoadCarSpeed)
								{ // preRoadCar能过路口
									// 比较两者的curPos
									if (firstCar.getCurPos() < firstCar.getCurPos())
									{// firstCar称为这条路的头车
										firstCar.setState(1);
									} else if (firstCar.getCurPos() == firstCar.getCurPos())
									{ // 位置相同比较laneID
									...
									{// firstCar的curPos在preRoadCar之后，preRoadCar是这条路的头车
										firstCar.setState(3);
									}
								} else
								{// preRoadCar不能过路口
							...
							{// firstCar不能过路口
								...
								{// preRoadCar不能过路口
									if (firstCar.getCurPos() < preRoadCar.getCurPos())
									{ // firstCar在前面
										// firstCar成为头车
										firstCar.setState(2);
									} else if (firstCar.getCurPos() == preRoadCar.getCurPos())
									{ // 位置相同，比较laneID
										if (firstCar.getLaneID() < firstCar.getLaneID())
										{ // firstCar成为头车
											firstCar.setState(2);
									...
									{// firstCar的curPospre在RoadCar之后，preRoadCar是这条路的头车
										firstCar.setState(4);
									}
								}
							}
	...
	protected static void UpdateRoadForCarsAtState2Super(String firstCarID)
	{
		...
		for (int i = 0; i < road.getLanesNum(); i++)
		{
			if (carInLane.get(i).carsInLane.size() > 0)
			{
				...
				UpdateLaneStateForCarsAtState2(carInLane.get(i).carsInLane.getFirst().getCarID(), newFirstCarID,
						followingState, carInLane);// 找出每个lane上的第一辆车
			} else
			{// 这个Lane上没有车，继续
				continue;
			}
	...
	protected static void UpdateLaneStateForCarsAtState2(String laneFirstCarID, String newFirstCarID,
			int followingState, LinkedList<Lane> carInLane)
	{
	 ...
	 *            车当前所在的路
	 * @param state
	 *            要将车更改为的数字状态，如果是-2则不更改此状态
	 * @param setHasArrangedOrNot
	 *            true则要更改此属性
	 * @param setSheng
	 *            true则要更改此属性，变为0
	 * @param setCurPos
	 ...
	 * 
	 * @param firstCarID
	 *            状态为5false的车，排在carInRoad的头头
	 * @version 2019.4.13
	 */
	protected static void updateCarsAtState5(String firstCarID)
	{
		Car firstCar = Main.MapCar.get(firstCarID);
	...
	/**
	 * 如果他要到家，他就变回1false了，后面车变成3false； 如果它发现有路可以走了，他就变回1false了，后面车变成3false；
