...
public class RunUtil2 {
	...
	protected static boolean[] checkIDPriority(Car car, Road road, 
		...
		int i = 0;
		for (i = size; i > 0; i--) {// 先拿出来的在lane4上的一系列车
...
//				if(cars.getLast().getCarID().equals("17944"))
//					System.out.println("出现回滚");
			
			
			while (cars.size() > 0 && cars.getLast().getPriority() == car.getPriority()// 这个车也是刚从车库v提出来的而且，这个车的ID比现在安排的车大
					&& Integer.valueOf(cars.getLast().getCarID()) > Integer.valueOf(car.getCarID())) {// 那这个车要回退的
				// 先把这些要退回的车辆的信息更新一下
				// 提取这辆车
...
//			car.setCurFromCrossID(carVir.getCurFromCrossID());
//			car.setPriority(carVir.getPriority());
//			car.setCurToCrossID(carVir.getCurToCrossID());
...
//		if(!flag)
//			car=virtualCarsHashMap.get(car.getCarID());
	...
	protected static boolean InsertFreshCarToRoad(Car c, Road road,
		...
		// car.setRealVelocity(nextLaneVel);
...
//				System.out.println(Main.problemCar+"coming!");
	...
	protected static Car minCarCurPos(Car[] carList) {
		...
		int j = 0;
		for (; j < carList.length; j++) {// 先取一个不为null的车作为CurPos的最小车
			if (carList[j] != null) {
				minCar = carList[j];
				break;
			}
		}
		for (int i = j + 1; i < carList.length; i++) {// 比较得到curPos最小车
			if (carList[i] == null)
				continue;
...
//		// int maxSpeed = Math.min(firstCar.getMaxVelocity(),
//		// carInRoad.getMaxRoadVelocity());
//
...
//		int state = 0;
//		int cariSpeed = 0;
//		if (firstCar.getState() == 5) {
//			state = 4;
//		} else {// firstCar.getState()==5
...
//	}
	...
	protected static Road findNextCross(Car car, int maxRoadLength) {
		...
		while (crossIter.hasNext()) {
			...
			if (s.getCrossID().equals(cross.getCrossID())) {// 初始化出发节点,我重写了equals
		...
		while (!t.isKnown) {
			...
			for (Road road : roadsList) {// 从该节点出发标记其相邻节点
	...
	protected static boolean deleteCrossFromUnknown(List<Cross> unknown, Car car, List<Road> roads, Road preRoad) {
		...
		for (int i = 0; i < roads.size(); i++) {
			...
			if (preRoad != null && (road.equals(preRoad))) {// 不走回头路
				flag[i] = -1;
			} else {
	...
	protected static Road findRoad(String crossSID, String crossTID) {
		...
		// if(roadID.equals("-1")) return null;
	...
	protected static void UpdateLaneForCars(Car firstCar) {
		...
		if (carInLaneNum > 1) {
			for (int i = 1; i < carInLaneNum; i++) {
				// ------case1:没有安排过的车
				if (carsInLane.get(i).isHasArrangedOrNot() == false) {
					if (carsInLane.get(i).getCurPos() - maxSpeed <= carsInLane.get(i - 1).getCurPos()) {// 能追上前车（前车已经更新到t3时刻了）
						carsInLane.get(i).setCurPos(carsInLane.get(i - 1).getCurPos() + 1);
					} else {// 如果不能追上前车，能跑多远跑多远
...
//						}
	...
	protected static void UpdateRoadForCarsAtState2Super(String firstCarID) {
		...
		if (mapFirstCarInLanes.isEmpty()) { // 如果这条路上没车
			System.out.println("map是空的");
		}
		// **********更新位置
		for (int i = 0; i < road.getLanesNum(); i++) {
			if (mapFirstCarInLanes.containsKey(i)) { // 如果这个lane上有车，更新这个lane上的车的位置
...
//		LinkedList<String> newCarsInRoad = getCarInRoad(road, firstCar.getCurFromCrossID());
...
//				}
	...
	protected static HashMap<Integer, String> findFirstCarInLanes(String firstCarID) {
		...
		while (carIt.hasNext()) {
			...
			if (!mapFirstCarInLanes.containsKey(cariLaneID)) {// 如果这个lane是第一次遍历到的
				mapFirstCarInLanes.put(cariLaneID, s);// 把这个carID放进map
			} else {
				continue;
			}
			if (mapFirstCarInLanes.size() == road.getLanesNum()) {// 每个lane都找到了就跳出循环
				break;
			}
		}
...
//		Car firstCar = Main.MapCar.get(firstCarID);
//		Road road = Main.MapRoad.get(firstCar.getRoadID());
//	    int lanesNum = road.getLanesNum();
//		LinkedList<Lane> lanes;
//	    //判断方向
//	    if(road.isDuplex() && road.getFromCrossID().equals(firstCar.getToCrossID())) {
//	    	lanes = road.getBackwardLane();
//	    }else {
//	    	lanes = road.getForwardLane();
//	    }
//		
//	    LinkedList<Car> carsInLanei;
//	    for(int i=0;i<lanesNum;i++) {
//	    	carsInLanei=lanes.get(i).carsInLane;
//	    	if(!carsInLanei.isEmpty()) {
//	    		mapFirstCarInLanes.put(i, carsInLanei.getFirst().getCarID());
//	    	}
//	    }
//	    
//		return mapFirstCarInLanes;
...
//				// ------case1:没有安排过的车
//				if (carsInLane.get(i).isHasArrangedOrNot() == false) {
//					if (carsInLane.get(i).getCurPos() >= cariSpeed) {// 该车原来不能过路口
//						if (carsInLane.get(i).getCurPos() - cariSpeed <= carsInLane.get(i - 1).getCurPos()) {// 能追上前车（前车已经更新到t3时刻了）
//							carsInLane.get(i).setCurPos(carsInLane.get(i - 1).getCurPos() + 1);
//						} else {// 如果不能追上前车，能跑多远跑多远
//							carsInLane.get(i).setCurPos(carsInLane.get(i).getCurPos() - cariSpeed);
...
//	}
	...
	protected static Car[] ThroughCar(Road road, Car[] carList, LinkedList<Lane> laneList, int[] carIndex) {
		...
		int j = 0;
		for (; j < carList.length; j++) {// 先取第一个不为null的车
			Car car = carList[j];
			if (car != null) {
	...
	protected static void canThrough(Road road, Car car, LinkedList<Lane> laneList, int[] carIndex) {
		...
		if (!carsInLane.isEmpty()) {
			if (carIn > 0 && !carsInLane.get(carIn - 1).isCanThrough()) {// 有前车且前车不能通过路口
	...
	public static void sortCarsOfState1(ArrayList<Car> cars) {
		...
		if (cars != null && cars.size() != 0) {
			for (int i = 0; i < cars.size(); i++) {
				Car car = cars.get(i);
				if(car.getCarID().equals("10037"))
					System.out.println("");;
				Road r=Main.MapRoad.get((car.getRoadID()));
				if (car.getCurToCrossID().equals(car.getToCrossID())) {//要到家的车
					car.setPriority(3);
					carsList.add(car);//参与排序
				} 
				else if(!car.getNextRoadID().equals("-1")) {//过路口但不许要重新找路
					...
					if (toRoad == null) {//找不到路则该车状态变为5，未安排的后车状态都变为5
			...
			for (int i = 0; i < carsList.size(); i++) {//优先级相同的车辆按ID升序排列
				int j = i + 1;
		...
		for (int j = 0; j < lanes.size(); j++) {
			...
			for (int y = 0; y < carsInLane.size(); y++) {
				...
				if(state!=-2) {
					...
					if(state==4) Main.numOf2++;
					if(state==5&&setHasArrangedOrNot) Main.numOf5+=2;
					if(state==5&&!setHasArrangedOrNot) Main.numOf5++;
				}
